...
public:
  ...
  virtual Parser* BuildTypeDef(std::string path) = 0;
  virtual Parser* BuildDepAnal(std::string path) = 0;
};
...
public:
  ...
  bool parse();
  bool next();
  void setAST(AbstractSyntaxTree* pAST) { ast = pAST; }
  void showAST(std::string filename) { ast->showAST(filename); }
private:
